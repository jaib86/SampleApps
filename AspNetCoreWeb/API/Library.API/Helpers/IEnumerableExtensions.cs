...
namespace Library.API.Helpers
{
    public static class IEnumerableExtensions
    {
        public static IEnumerable<ExpandoObject> ShapeData<TSource>(this IEnumerable<TSource> source, string fields)
        {
            ...
            // Reflection is expensive, so rather doing it for each object in the list,
            ...
            else
            {
                ...
                foreach (var field in fieldsAfterSplit)
                {
                    ...
                    {
                        throw new LibraryException($"Property {propertyName} wasn't found on {typeof(TSource)}");
                    }
            ...
            {
                // create an ExpandoObect that will hold the selected properties & values
                var dataShapedObject = new ExpandoObject();

                // Get the value of each property we have to run.
